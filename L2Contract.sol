...
interface RaffleL1 {
    function getPrize(uint256 _tokenId, address _winner) external;
}
...
contract RaffleL2 is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
    ...
    {
        treasury = payable(0x73C32AC751EaaAA73cf3E04338a0d5FC07662279);
    ...
    function getAllRaffles() external view returns (RaffleData[] memory) {
        uint256 length = raffleCounter;
        RaffleData[] memory raffleDataList = new RaffleData[](length);

        for (uint256 i = 0; i < length; i++) {
            RaffleInfo storage raffle = raffles[i];
            RaffleData memory raffleData;

            raffleData.domainName = raffle.domainName;
            raffleData.minimumAmount = raffle.minimumAmount;
            raffleData.deadline = raffle.deadline;
            raffleData.prize = raffle.prize;
            raffleData.totalTickets = raffle.totalTickets;
            raffleData.ticketPrice = raffle.ticketPrice;
            raffleData.requestId = raffle.requestId;
            raffleData.completed = raffle.completed;

            raffleData.isParticipant = false;
            for (uint256 j = 0; j < raffle.participants.length; j++) {
                if (raffle.participants[j] == msg.sender) {
                    raffleData.isParticipant = true;
                    break;
                }
            }

            raffleData.numberOfTickets = ticketsBought[i][msg.sender];
            raffleData.domainOwner = raffle.domainOwner;
            raffleData.winner = raffle.winner;
            raffleData.taken = raffle.taken;

            raffleDataList[i] = raffleData;
        }

        return raffleDataList;
    }
    ...
    function setWinner(uint256 raffleId) external {
        ...
        if (totalEmitted < raffles[raffleId].minimumAmount) {
            ...
            bool isFulfilled = s_requests[raffles[raffleId].requestId]
                .fulfilled;

            require(isFulfilled, "The request has not been fulfilled yet.");

            emit RequestStatusChecked(
                raffleId,
                isFulfilled,
                raffles[raffleId].requestId
            ); // Evento dopo il controllo dello stato della richiesta
            ...
            if (isFulfilled) {
                ...
                // Calcola il 10% del totale emesso
                uint256 treasuryAmount = totalEmitted / 10;
                // Calcola il resto del totale emesso
                uint256 oldOwnerAmount = totalEmitted - treasuryAmount;
                // Trasferisci il 10% al treasury
                treasury.transfer(treasuryAmount);

                address payable oldOwner = payable(
                    raffles[raffleId].domainOwner
                );
                // Trasferisci il resto al vecchio owner
                oldOwner.transfer(oldOwnerAmount);
            }
        }
    }

    function claimPrizeInL1(uint256 raffleId) public returns (uint256) {
        require(raffles[raffleId].taken == false, "Raffle has already taken");
        require(
            raffles[raffleId].winner == msg.sender,
            "You are not the winner"
        );
        bytes memory data = abi.encodeWithSelector(
            RaffleL1.getPrize.selector,
            raffles[raffleId].prize,
            raffles[raffleId].winner
        );
        raffles[raffleId].taken = true;
